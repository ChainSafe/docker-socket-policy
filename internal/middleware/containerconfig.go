...
// ContainerConfigMutator enforces container configuration from the policy.
// It overwrites any conflicting values in the request body.
...
func (m *ContainerConfigMutator) Mutate(r *http.Request, p *policy.Policy, body map[string]interface{}) {
	...
	if cc.NetworkMode == "" && cc.RestartPolicy == "" && len(cc.SecurityOptions) == 0 {
		return
	}
